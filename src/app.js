...
$(function() {
  ...
  $('[data-route="workspace"] [id="line"]').click(function() {
  ...
  cnc.updateView = function() {
    ...
    totalLines = $('[data-route="workspace"] [id="gcode"]').text().split("\n").length || $('[data-route="workspace"] [id="gcode"]').val().split("\n").length;
  ...
  $('[data-route="workspace"] select[data-name="select-file"]').change(function() {
    cnc.loadGCode();
  });
  ...
  cnc.probe = function() {
    if (confirm("Ensure the probe is in place and connected, then touch OK.") == true) {
